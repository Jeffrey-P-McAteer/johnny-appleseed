...
# Third-party (provided by the uv inline-script dependency block above).
...
def ovmf_to_qemu_args(code_path: str):
    ...
    if vars_path:
        ...
        os_temp_dir_vars_file = os.path.join(tempfile.gettempdir(), 'testbed-mac-'+os.path.basename(vars_path))
...
class AnyUserAuthorizer(DummyAuthorizer):
    ...
    def has_perm(self, username, perm, path=None):
        return perm in self._perm

    def get_msg_login(self, username):
        return f'Logged in as {username!r} with full read/write.'
...
class FtpShare:
    ...
    and any-login read+write makes it two-way (the guest can upload logs back to
    the host).
    ...
    fixed passive_ports range, all reachable from the guest as 10.0.2.2:<port>
    without any QEMU hostfwd. (If your libslirp routes 10.0.2.2 somewhere other
    than loopback and the guest can't connect, change host to '0.0.0.0'.)
    """
    def __init__(self, root, host='127.0.0.1', port=2121,
        ...
        handler = FTPHandler
        handler.authorizer = AnyUserAuthorizer(root)
        handler.masquerade_address = masquerade
        handler.passive_ports = list(passive_ports)
        handler.banner = 'johnny-appleseed testbed FTP (any login, read+write)'
...
vm_data_folder = os.path.join(testbed_folder, 'vm-data-macos-intel')
...
install_iso = glob_for_nonempty_file_fatal(
  vm_data_folder, '*.iso',
  f'''
Please download a macOS installer .iso file
...
vm_qcow2s = glob_for_nonempty_files(vm_data_folder, '*[mM]ac[oO][sS]*.qcow2')
if len(vm_qcow2s) > 1:
...
if len(vm_qcow2s) < 1:
  ...
  vm_qcow2 = os.path.join(vm_data_folder, 'MacOS-Test-VM.qcow2') # Assignment: Default qcow2 name
  pretty_cmd(
    qemu_img_exe, 'create', '-f', 'qcow2', vm_qcow2, '185G',
    cwd=vm_data_folder
  )
...
macos_osk_string_file = os.path.join(vm_data_folder, 'macos-osk-string.txt')
if not os.path.exists(macos_osk_string_file):
  with open(macos_osk_string_file, 'w') as fd:
    fd.write('')

macos_osk_string = ''
with open(macos_osk_string_file, 'r') as fd:
  macos_osk_string = fd.read().strip()
while len(macos_osk_string) < 10:
  print('Useful link: https://github.com/random-archer/apple_OSX-KVM/blob/94bb61124a5986ada33c160f8c75b62ca97cfce3/boot-passthrough.sh#L46')
  _unused = input(f'The file {macos_osk_string_file} needs the OSK string added, please legally aquire it and place in the file, then hit enter to continue')
  with open(macos_osk_string_file, 'r') as fd:
    macos_osk_string = fd.read().strip()
